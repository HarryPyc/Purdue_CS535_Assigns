...
Scene* InitMainScene()
{
	...
	scene->MainCamera = Camera(fvec4(0, 2, 4, 1), fvec4(0, 0, 0, 1), fvec4(0, 1, 0, 0), 60.f, float(WINDOW_WIDTH) / float(WINDOW_HEIGHT), 0.1f, 100.f);
	...
	//Mesh Cube("asset/model/cube.obj");  Cube.material.color = fvec4(1.f, 1.f, 1.f, 1.0f); 
	//Cube.S = Scale(fvec4(1.0f)); //Cube.T = Translate(fvec4(1,0,0,0));
	//Cube.enableLight = false;
	//scene->meshList.push_back(Cube);

	Mesh Fish;
	Fish.texture = new Texture("asset/texture/AmagoT.bmp");
	Fish.LoadObj("asset/model/Amago0.obj"); Fish.S = Scale(fvec4(10, 10, 10, 0)); Fish.T = Translate(fvec4(0, 0.8f, 0.0f, 0));
	...
	PointLight light = PointLight(fvec4(2.f, 2.f, 2.f, 1), fvec4(1.f));
