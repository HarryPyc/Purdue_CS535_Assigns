...
Graphics::Graphics(int _width, int _height)
{
	...
	meshList.push_back(Fish); meshList.push_back(Cube1); meshList.push_back(Cube2); meshList.push_back(Cube3); meshList.push_back(Cube4);
}
...
void Graphics::run()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (frames++ >= 150 && frames < 300) {
			cam->Interpolate(cam0, cam1, frames - 150, 150);
		}
		for (int i = 1; i < meshList.size(); i++) {
			fbo->DrawMesh(cam, meshList[i],DRAW_FILL);
		}
		fbo->DrawMesh(cam, meshList[0], DRAW_LINES);
